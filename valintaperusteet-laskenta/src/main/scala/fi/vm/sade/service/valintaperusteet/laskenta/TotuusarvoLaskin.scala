...
protected[laskenta] class TotuusarvoLaskin(private val laskin: Laskin) extends LaskinFunktiot {
  ...
  protected[laskenta] def laskeTotuusarvo(
    ...
    def muodostaKoostettuTulos(fs: Seq[Totuusarvofunktio], trans: Seq[Boolean] => Boolean) = {
      val (tulokset, tilat, historiat) = fs.reverse.foldLeft(
        (Nil, Nil, ListBuffer()): (List[Option[Boolean]], List[Tila], ListBuffer[Historia])
      )((lst, f) => {
      ...
      (
        totuusarvo,
        tilat,
        Historia("Koostettu tulos", totuusarvo, tilat, Some(historiat.toList), None)
      )
    }
    ...
    val (laskettuTulos, tilat, hist): (Option[Boolean], List[Tila], Historia) = laskettava match {
      ...
      case HaeTotuusarvo(konvertteri, oletusarvo, valintaperusteviite, _, _, _, _, _) =>
        ...
        val (tulos, tila) = laskin.haeValintaperuste[Boolean](
          valintaperusteviite,
          laskin.hakemus.kentat,
          s =>
            suoritaOptionalKonvertointi[Boolean](
              string2boolean(s, valintaperusteviite.tunniste),
              konv
            ),
          oletusarvo
        )
        (
          tulos,
          tila,
          Historia(HAETOTUUSARVO, tulos, tila, None, Some(Map("oletusarvo" -> oletusarvo)))
        )
      case NimettyTotuusarvo(nimi, f, _, _, _, _, _) =>
        val (tulos, tilat, h) = muodostaYksittainenTulos(f, b => b)
        (
          tulos,
          tilat,
          Historia(NIMETTYTOTUUSARVO, tulos, tilat, Some(List(h)), Some(Map("nimi" -> Some(nimi))))
        )

      case Hakutoive(n, _, _, _, _, _) =>
        val onko = Some(
          laskin.hakemus.onkoHakutoivePrioriteetilla(laskin.hakukohde.hakukohdeOid, n)
        )
        val tilat = List(new Hyvaksyttavissatila)
      ...
      case HakutoiveRyhmassa(n, ryhmaOid, _, _, _, _, _) =>
        val onko = Some(
          laskin.hakemus
            .onkoHakutoivePrioriteetilla(laskin.hakukohde.hakukohdeOid, n, Some(ryhmaOid))
        )
        val tilat = List(new Hyvaksyttavissatila)
        (
          onko,
          tilat,
          Historia(HAKUTOIVERYHMASSA, onko, tilat, None, Some(Map("prioriteetti" -> Some(n))))
        )

      case Hakukelpoisuus(_, _, _, _, _) =>
        ...
        (
          ...
          Historia(
            ...
            tilat,
            None,
            Some(Map("hakukohde" -> Some(laskin.hakukohde.hakukohdeOid)))
          )
        )

      case Demografia(_, _, _, _, _, tunniste, prosenttiosuus) =>
        if (laskin.laskentamoodi != Laskentamoodi.VALINTALASKENTA) {
          val moodi = laskin.laskentamoodi.toString
          moodiVirhe(
            s"Demografia-funktiota ei voida suorittaa laskentamoodissa $moodi",
            "Demografia",
            moodi
          )
        } else {
          val ensisijaisetHakijat = laskin.kaikkiHakemukset.count(
            _.onkoHakutoivePrioriteetilla(laskin.hakukohde.hakukohdeOid, 1)
          )

          val omaArvo = laskin.hakemus.kentat.toSeq
            .map(e => e._1.toLowerCase -> e._2)
            .toMap
            .get(tunniste.toLowerCase)
          val tulos = Some(
            if (ensisijaisetHakijat == 0) false
            else {
              val samojenArvojenLkm = laskin.kaikkiHakemukset.count(h =>
                h.onkoHakutoivePrioriteetilla(laskin.hakukohde.hakukohdeOid, 1) &&
                  h.kentat.toSeq
                    .map(e => e._1.toLowerCase -> e._2)
                    .toMap
                    .get(tunniste.toLowerCase) == omaArvo
              )

              val vertailuarvo = BigDecimal(samojenArvojenLkm).underlying
                .divide(BigDecimal(ensisijaisetHakijat).underlying, 4, RoundingMode.HALF_UP)
              vertailuarvo.compareTo(
                prosenttiosuus.underlying.divide(HUNDRED.underlying, 4, RoundingMode.HALF_UP)
              ) != 1
            }
          )
          ...
          (
            ...
            Historia(
              ...
              List(tila),
              None,
              Some(Map("avain" -> Some(tunniste), "valintaperuste" -> omaArvo))
            )
          )
        }

      case HaeMerkkijonoJaKonvertoiTotuusarvoksi(
            konvertteri,
            oletusarvo,
            valintaperusteviite,
            _,
          ...
          case a: Arvokonvertteri[_, _] =>
            ...
            if (konv.isEmpty) {
              (
                None,
                virheet,
                Historia(HAEMERKKIJONOJAKONVERTOITOTUUSARVOKSI, None, virheet, None, None)
              )
            } else {
              val (tulos, tila) = laskin.haeValintaperuste[Boolean](
                valintaperusteviite,
                laskin.hakemus.kentat,
                s =>
                  suoritaKonvertointi[String, Boolean](
                    (Some(s), new Hyvaksyttavissatila),
                    konv.get.asInstanceOf[Arvokonvertteri[String, Boolean]]
                  ),
                oletusarvo
              )
              (
      ...
      case HaeMerkkijonoJaVertaaYhtasuuruus(
            oletusarvo,
            valintaperusteviite,
            vertailtava,
            _,
        ...
        val (tulos, tila) = laskin.haeValintaperuste[Boolean](
          valintaperusteviite,
          laskin.hakemus.kentat,
          s => (Some(vertailtava.trim.equalsIgnoreCase(s.trim)), List(new Hyvaksyttavissatila)),
          oletusarvo
        )
        (
          tulos,
      ...
      case Valintaperusteyhtasuuruus(
            _,
            _,
            _,
            _,
            _,
            (valintaperusteviite1, valintaperusteviite2)
          ) =>
        val (arvo1, tilat1) = laskin.haeValintaperuste[String](
          valintaperusteviite1,
          laskin.hakemus.kentat,
          s => (Some(s.trim.toLowerCase), List(new Hyvaksyttavissatila)),
          None
        )
        val (arvo2, tilat2) = laskin.haeValintaperuste[String](
          valintaperusteviite2,
          laskin.hakemus.kentat,
          s => (Some(s.trim.toLowerCase), List(new Hyvaksyttavissatila)),
          None
        )
        ...
        (
          ...
          Historia(
            ...
            tilat,
            None,
            Some(Map("tunniste1" -> arvo1, "tunniste2" -> arvo2))
          )
        )
    }
