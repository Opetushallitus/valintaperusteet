...
protected[laskenta] class Laskin private(val hakukohde: Hakukohde,
  ...
  protected[laskenta] def haeValintaperuste[T](valintaperusteviite: Valintaperuste, kentat: Kentat,
  ...
  protected[laskenta] def laskeLukuarvo(laskettava: Lukuarvofunktio, iteraatioParametrit: Map[Class[_ <: IteraatioParametri], IteraatioParametri]): Tulos[BigDecimal] = {
    ...
    val (laskettuTulos: Option[BigDecimal], tilat: Seq[Tila], historia: Historia) = laskettava match {
      ...
      case Jos(ehto, thenHaara, elseHaara, _, _,_,_,_,_) =>
        val ehtoTulos = new TotuusarvoLaskin(this).laskeTotuusarvo(ehto, iteraatioParametrit)
      ...
      case f@IteroiAmmatillisenTutkinnonYtoOsaAlueet(lapsiF, _, _, _, _, _, _, _) =>
        ...
        if (tutkinnonYtoOsaAlueenValitsija.exists(p => p.isInstanceOf[AmmatillisenTutkinnonYtoOsaAlueenValitsija])) {
          ...
          val osienMaara = KoskiLaskenta.laskeAmmatillisenTutkinnonYtoOsaAlueet(tutkinnonValitsija, hakemus)
          Laskin.LOG.info(s"Hakemuksen ${hakemus.oid} hakijan tutkinnolle ${tutkinnonValitsija} löytyi ${osienMaara} ammatillista perustutkinnon osaa.")
      ...
      case Hylkaa(f, hylkaysperustekuvaus, _, _,_,_,_,_) =>
        new TotuusarvoLaskin(this).laskeTotuusarvo(f, iteraatioParametrit) match {
          case Tulos(tulos, tila, historia) =>
