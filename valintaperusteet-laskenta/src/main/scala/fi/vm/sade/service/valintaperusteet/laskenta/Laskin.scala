...
object Laskin {
  ...
  def wrap(hakemus: Hakemus, historia: Historia) = {
    ...
    Historia(name, historia.tulos, Some(List(historia)), Some(v))
  }
}
...
class Laskin(hakukohde: String, hakemus: Hakemus) {
  ...
  def laske(laskettava: Totuusarvofunktio): (Option[Boolean], Tila, Historia) = {
    ...
    def muodostaKoostettuTulos(fs: Seq[Totuusarvofunktio], trans: Seq[Boolean] => Boolean) = {
      ...
      (totuusarvo, tilat, Historia("Koostettu tulos", totuusarvo, Some(historiat.toList), None))
    }
    ...
    def muodostaVertailunTulos(f1: Lukuarvofunktio, f2: Lukuarvofunktio,
      ...
      (tulos, List(tila1, tila2), Historia("Vertailuntulos", tulos, Some(List(historia1, historia2)), None))
    }
    ...
    val (laskettuTulos, tilat, hist): Tuple3[Option[Boolean], List[Tila], Historia] = laskettava match {
      case Ja(fs, oid) => {
        ...
        (tulos, tilat, Historia("Ja", tulos, h.historiat, None))
      }
      case Tai(fs, oid) => {
        val (tulos, tilat, h) = muodostaKoostettuTulos(fs, lst => lst.exists(b => b));
        (tulos, tilat, Historia("Tai", tulos, h.historiat, None))
      }
      case Ei(fk, oid) => {
        val (tulos, tilat, h) = muodostaYksittainenTulos(fk, b => !b)
        (tulos, tilat, Historia("Ei", tulos, Some(List(h)), None))
      }
      ...
      case HaeTotuusarvo(konvertteri, oletusarvo, valintaperusteviite, oid) => {
        ...
        arvoOption match {
          case None => {
            ...
            (oletusarvo, List(tila), Historia("Hae totuusarvo (oletusarvo)", oletusarvo, None, Some(Map("oletusarvo" -> oletusarvo, "tunniste" -> Some(valintaperusteviite.tunniste)))))
          }
          case Some(arvo) => {
            ...
            (tulos, tila, Historia("Hae totuusarvo", tulos, None, Some(Map("oletusarvo" -> oletusarvo, "tunniste" -> Some(valintaperusteviite.tunniste)))))
          }
        }
      }
      case NimettyTotuusarvo(nimi, f, oid) => {
        val (tulos, tilat, h) = muodostaYksittainenTulos(f, b => b)
        (tulos, tilat, Historia("Nimetty totuusarvo", tulos, Some(List(h)), Some(Map("nimi" -> Some(nimi)))))
      }

      case Hakutoive(n, oid) => {
        val onko = Some(hakemus.onkoHakutoivePrioriteetilla(hakukohde, n));

        (onko, List(new Hyvaksyttavissatila), Historia("Hakutoive", onko, None, Some(Map("prioriteetti" -> Some(n)))))
      }
      ...
      case d: Demografia => {
        ...
        (arvoOption, List(new Hyvaksyttavissatila), Historia("Demografia", arvoOption, None, Some(Map("avain" -> Some(avain), "valintaperuste" -> valintaperuste))))
      }
      ...
      case HaeMerkkijonoJaKonvertoiTotuusarvoksi(konvertteri, oletusarvo, valintaperusteviite, oid) => {
        ...
        valintaperuste match {
          case Some(s) => {
            ...
            (tulos, tila, Historia("Hae merkkijono ja konvertoi totuusarvoksi", tulos, None, Some(Map("oletusarvo" -> oletusarvo))))
          ...
          case None => {
            ...
            (oletusarvo, List(tila), Historia("Hae merkkijono ja konvertoi totuusarvoksi (oletusarvo)", oletusarvo, None, Some(Map("oletusarvo" -> oletusarvo))))
          }
        }
      }
    }
  ...
  def laske(laskettava: Lukuarvofunktio): (Option[BigDecimal], Tila, Historia) = {
    ...
    def muodostaKoostettuTulos(fs: Seq[Lukuarvofunktio], trans: Seq[BigDecimal] => BigDecimal): (Option[BigDecimal], List[Tila], Historia) = {
      ...
      (lukuarvo, tulokset._2, Historia("Koostettu tulos", lukuarvo, Some(tulokset._3.toList), None))
    }
    ...
    val (laskettuTulos: Option[BigDecimal], tilat: Seq[Tila], historia: Historia) = laskettava match {
      ...
      case Summa(fs, oid) => {
        ...
        (tulos, tilat, Historia("Summa", tulos, h.historiat, None))
      }
      ...
      case Minimi(fs, oid) => {
        ...
        (tulos, tilat, Historia("Minimi", tulos, h.historiat, None))
      }

      case Maksimi(fs, oid) => {
        val (tulos, tilat, h) = muodostaKoostettuTulos(fs, ds => ds.max)
        (tulos, tilat, Historia("Maksimi", tulos, h.historiat, None))
      }
      ...
      case Jos(ehto, thenHaara, elseHaara, oid) => {
        ...
        (tulos, tilat, Historia("Jos", tulos, Some(List(historia0, historia1, historia2)), None))
      }
      ...
      case HaeLukuarvo(konvertteri, oletusarvo, valintaperusteviite, oid) => {
        ...
        arvoOption match {
          case None => {
            ...
            (oletustulos, oletustila, Historia("Hae lukuarvo (oletusarvo)", oletustulos, None, Some(Map("oletusarvo" -> oletustulos))))
          }
          case temp: Option[_] => {
            temp.get match {
      ...
      case HaeMerkkijonoJaKonvertoiLukuarvoksi(konvertteri, oletusarvo, valintaperusteviite, oid) => {
        ...
        valintaperuste match {
          ...
          case None => {
            ...
            (oletusarvo, List(tila), Historia("Hae merkkijono ja konvertoi lukuarvoksi (oletusarvo)", oletusarvo, None, Some(Map("oletusarvo" -> oletusarvo))))
          }
        }
