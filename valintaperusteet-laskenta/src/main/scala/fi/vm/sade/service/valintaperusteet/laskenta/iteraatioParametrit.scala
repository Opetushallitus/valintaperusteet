...
case class AmmatillisenTutkinnonOsat(tutkinnonOsat: Seq[Osasuoritus]) {
  def parametreiksi: Seq[AmmatillisenTutkinnonOsanValitsija] =
    tutkinnonOsat.zipWithIndex.map(AmmatillisenTutkinnonOsanValitsija.tupled)
}

case class AmmatillisenTutkinnonYtoOsaAlueenValitsija(
  ytoKoodi: String,
  ytoOsaSuoritus: Osasuoritus,
  osaAlueenIndeksi: Int
) extends IteraatioParametri {
  override val kuvaus: String =
    s"YTO:n ${ytoKoodi} osa-alue ${osaAlueenIndeksi + 1}: ${ytoOsaSuoritus.koulutusmoduulinNimiFi}"
  override val lyhytKuvaus: String =
    s"YTO:n ${ytoKoodi} osa-alue: ${ytoOsaSuoritus.koulutusmoduulinNimiFi}"
}

case class AmmatillisenTutkinnonYtoOsaAlueet(
  ytoKoodi: String,
  tutkinnonOsaAlueet: Seq[Osasuoritus]
) {
  def parametreiksi: Seq[AmmatillisenTutkinnonYtoOsaAlueenValitsija] =
    tutkinnonOsaAlueet.zipWithIndex.map(x =>
      AmmatillisenTutkinnonYtoOsaAlueenValitsija(ytoKoodi, x._1, x._2)
    )
}

case class LaskennanIteraatioParametrit(
  parametriListat: Map[Class[_ <: IteraatioParametri], Seq[IteraatioParametri]] = Map(),
  ...
  def asetaAvoinParametrilista[T <: IteraatioParametri](
    tyyppi: Class[T],
    parametrit: Seq[T]
  ): LaskennanIteraatioParametrit = {
  ...
  def avoinParametrilista: Seq[IteraatioParametri] =
    parametriListat.toSeq.filter(x => x._2.nonEmpty) match {
      case (_, lista) :: Nil => lista
      case Nil =>
        throw new IllegalArgumentException(
          s"Ei löytynyt avointa parametrilistaa iteraatioparametreista $this"
        )
      case _ =>
        throw new IllegalArgumentException(
          s"Löytyi useampi avoin parametrilista iteraatioparametreista $this"
        )
    }
  ...
  def sido(parametri: IteraatioParametri): LaskennanIteraatioParametrit = {
    parametri match {
      case p: AmmatillisenPerustutkinnonValitsija =>
        copy(
          ammatillisenPerustutkinnonValitsija = Some(p),
          parametriListat = listatIlman(classOf[AmmatillisenPerustutkinnonValitsija])
        )
      case p: AmmatillisenTutkinnonOsanValitsija =>
        copy(
          ammatillisenTutkinnonOsanValitsija = Some(p),
          parametriListat = listatIlman(classOf[AmmatillisenTutkinnonOsanValitsija])
        )
      case p: AmmatillisenTutkinnonYtoOsaAlueenValitsija =>
        copy(
          ammatillisenTutkinnonYtoOsaAlueenValitsija = Some(p),
          parametriListat = listatIlman(classOf[AmmatillisenTutkinnonYtoOsaAlueenValitsija])
        )
    }
  }

  private def listatIlman[T <: IteraatioParametri](
    poistettavaTyyppi: Class[T]
  ): Map[Class[_ <: IteraatioParametri], Seq[IteraatioParametri]] = {
