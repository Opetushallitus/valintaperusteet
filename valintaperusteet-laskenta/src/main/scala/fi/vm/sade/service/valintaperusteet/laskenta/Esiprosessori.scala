    ...
    prosessoitava match {
      case f: Esiprosessoiva => {
        ...
        if (ensisijaisetHakijat == 0) { // ei ensisijaisia hakijoita
          ...
          val vertailuarvo = new BigDecimal(samatArvotLkm).divide(new BigDecimal(ensisijaisetHakijat)) //hakemukset.size()
          val arvo = (vertailuarvo.compareTo(f.prosenttiosuus.divide(new BigDecimal("100.0"))) != 1).toString
