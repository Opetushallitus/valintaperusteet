...
object KoskiLaskenta {
  ...
  def laskeAmmatillisetTutkinnot(hakemus: Hakemus): Int = {
    if (hakemus.koskiOpiskeluoikeudet == null) {
      ...
      etsiValmiitTutkinnot(hakemus.koskiOpiskeluoikeudet, ammatillisenHuomioitavaOpiskeluoikeudenTyyppi).size
    }
  }
  ...
  private def haeYtoArvosana(ammatillisenPerustutkinnonValitsija: AmmatillisenPerustutkinnonValitsija,
    ...
    val oikeaOpiskeluoikeus: Json = etsiValmiitTutkinnot(opiskeluoikeudet, ammatillisenHuomioitavaOpiskeluoikeudenTyyppi)(ammatillisenPerustutkinnonValitsija.tutkinnonIndeksi)
