...
trait AmmatillisetIterointiFunktiot {
  ...
  protected def iteroiAmmatillisetTutkinnot(
    ...
    if (iteraatioParametrit.ammatillisenPerustutkinnonValitsija.isDefined) {
      throw new IllegalStateException(
        s"Ei voi iteroida iteraatioparametrilla ${iteraatioParametrit.ammatillisenPerustutkinnonValitsija} uudestaan ammatillisten tutkintojen yli"
      )
    } else {
      ...
      Laskin.LOG.info(
        s"Hakemuksen ${laskin.hakemus.oid} hakijalle löytyi $tutkintojenMaara ammatillista perustutkintoa: $tutkinnot, " +
          s"joista käytetään laskennassa ${tutkintojenIterointiParametrit.size}:  ${tutkintojenIterointiParametrit
            .map(_.kuvaus)}."
      )
      Laskin.LOG.info(
        s"${classOf[IteroiAmmatillisetTutkinnot].getSimpleName}-funktion parametrit: " +
      ...
      val uudetIteraatioParametrit = iteraatioParametrit.asetaAvoinParametrilista(
        classOf[AmmatillisenPerustutkinnonValitsija],
        tutkintojenIterointiParametrit
      )
      ...
      val tulos: Tulos[BigDecimal] = if (tutkintojenIterointiParametrit.nonEmpty) {
        ...
        Tulos(
          ...
          Historia(ITEROIAMMATILLISETTUTKINNOT, None, Nil, None, None)
        )
      }

      val ammatillistenFunktioidenTulostenTiivistelmat: ListMap[String, Option[Any]] =
        tallennetutTuloksetHistoriaaVarten

      val tilalista = List(tulos.tila)
      val avaimet = ListMap(
        "Valmistumisen takaraja" -> Some(
          suomalainenPvmMuoto.format(iterointiFunktio.valmistumisenTakaraja)
        ),
        ...
        "Ammatilliset perustutkinnot" -> Some(
          tutkintojenIterointiParametrit.map(_.kuvaus).mkString("; ")
        )
      ) ++
  ...
  protected def iteroiAmmatillisetTutkinnonOsat(
    iteraatioParametrit: LaskennanIteraatioParametrit,
    iterointiFunktio: IteroiAmmatillisetTutkinnonOsat
  ): (Option[BigDecimal], List[Tila], Historia) = {
    val tutkinnonValitsija: AmmatillisenPerustutkinnonValitsija =
      ammatillisenTutkinnonValitsija(iteraatioParametrit, iterointiFunktio)

    if (iteraatioParametrit.ammatillisenTutkinnonOsanValitsija.isDefined) {
      throw new IllegalStateException(
        s"Ei voi iteroida iteraatioparametrilla ${iteraatioParametrit.ammatillisenTutkinnonOsanValitsija} uudestaan ammatillisen tutkinnon osien yli"
      )
    } else {
      val tutkinnonOsat: Seq[Osasuoritus] =
        KoskiLaskenta.etsiAmmatillisenTutkinnonOsat(tutkinnonValitsija, laskin.hakemus)
      val osienMaara = tutkinnonOsat.size
      Laskin.LOG.info(
        s"Hakemuksen ${laskin.hakemus.oid} hakijan tutkinnolle ${tutkinnonValitsija.kuvaus} löytyi $osienMaara ammatillista perustutkinnon osaa."
      )

      val tutkinnonOsienIterointiParametrit: Seq[AmmatillisenTutkinnonOsanValitsija] =
        AmmatillisenTutkinnonOsat(tutkinnonOsat).parametreiksi

      val uudetIteraatioParametrit = iteraatioParametrit.asetaAvoinParametrilista(
        classOf[AmmatillisenTutkinnonOsanValitsija],
        tutkinnonOsienIterointiParametrit
      )
      ...
      val tulos: Tulos[BigDecimal] = if (tutkinnonOsienIterointiParametrit.nonEmpty) {
        ...
        Tulos(
          None,
          new Hyvaksyttavissatila,
          Historia(ITEROIAMMATILLISETOSAT, None, Nil, None, None)
        )
      }

      val tilalista = List(tulos.tila)
      val avaimet = Map(
        s"ammatillisen perustutkinnon ${tutkinnonValitsija.lyhytKuvaus} osien määrä" -> Some(
          osienMaara
        ),
        s"ammatillisen perustutkinnon ${tutkinnonValitsija.lyhytKuvaus} osat" -> Some(
          tutkinnonOsienIterointiParametrit.map(_.kuvaus).mkString("; ")
        )
      )
      (
        tulos.tulos,
  ...
  protected def iteroiAmmatillisenTutkinnonYtoOsaAlueet(
    iteraatioParametrit: LaskennanIteraatioParametrit,
    ...
    if (iteraatioParametrit.ammatillisenTutkinnonYtoOsaAlueenValitsija.isDefined) {
      throw new IllegalStateException(
        s"Ei voi iteroida iteraatioparametrilla ${iteraatioParametrit.ammatillisenTutkinnonYtoOsaAlueenValitsija} uudestaan ammatillisen tutkinnon osien yli"
      )
    } else {
      val ytoKoodi = iterointiFunktio.valintaperusteviite.tunniste
      val ytoOsaAlueet = KoskiLaskenta.haeAmmatillisenTutkinnonYtoOsaAlueet(
        tutkinnonValitsija,
        ytoKoodi,
        laskin.hakemus
      )
      ...
      Laskin.LOG.info(
        s"Hakemuksen ${laskin.hakemus.oid} hakijan tutkinnon ${tutkinnonValitsija.lyhytKuvaus} YTOlle $ytoKoodi löytyi ${ytoOsaAlueet.size} YTOn osa-aluetta."
      )

      val ytoOsaAlueidenIteraatioParametrit: Seq[AmmatillisenTutkinnonYtoOsaAlueenValitsija] =
        AmmatillisenTutkinnonYtoOsaAlueet(ytoKoodi, ytoOsaAlueet).parametreiksi

      val uudetIteraatioParametrit = iteraatioParametrit.asetaAvoinParametrilista(
        classOf[AmmatillisenTutkinnonYtoOsaAlueenValitsija],
        ytoOsaAlueidenIteraatioParametrit
      )
      ...
      val tulos: Tulos[BigDecimal] = if (ytoOsaAlueidenIteraatioParametrit.nonEmpty) {
        ...
        Tulos(
          None,
          new Hyvaksyttavissatila,
          Historia(ITEROIAMMATILLISETYTOOSAALUEET, None, Nil, None, None)
        )
      }

      val tilalista = List(tulos.tila)
      val avaimet = Map(
        s"ammatillisen perustutkinnon ${tutkinnonValitsija.lyhytKuvaus} YTOn $ytoKoodi osa-alueiden määrä" -> Some(
          ytoOsaAlueet.size
        ),
        s"ammatillisen perustutkinnon ${tutkinnonValitsija.lyhytKuvaus} YTOn $ytoKoodi osa-alueet" -> Some(
          ytoOsaAlueidenIteraatioParametrit.map(_.kuvaus).mkString("; ")
        )
      )
      (
        tulos.tulos,
  ...
  private def tallennetutTuloksetHistoriaaVarten: ListMap[String, Option[String]] = {
    val tuloksetSuomenkielistenNimienKanssa: List[(String, Option[String])] =
      laskin.funktioTulokset.toList.map {
