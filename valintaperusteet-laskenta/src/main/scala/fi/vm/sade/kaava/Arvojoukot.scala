package fi.vm.sade.kaava

/**
 * Created by kjsaila on 17/03/14.
 */
object Arvojoukot {

  val YO_OPPIAINEET = Array(
    "O"->"Äidinkielen koe, ruotsi",
    "O5"->"Ruotsi toisena kielenä",
    "A"->"Äidinkielen koe, suomi",
    "A5"->"Suomi toisena kielenä",
    "BA"->"Ruotsi, pitkä oppimäärä",
    "BB"->"Ruotsi, keskipitkä oppimäärä",
    "CA"->"Suomi, pitkä oppimäärä",
    "CB"->"Suomi, keskipitkä oppimäärä",
    "CC"->"Suomi, lyhyt oppimäärä",
    "DC"->"Pohjoissaame, lyhyt oppimäärä",
    "EA"->"Englanti, pitkä oppimäärä",
    "EB"->"Englanti, keskipitkä oppimäärä",
    "EC"->"Englanti, lyhyt oppimäärä",
    "FA"->"Ranska, pitkä oppimäärä",
    "FB"->"Ranska, keskipitkä oppimäärä",
    "FC"->"Ranska, lyhyt oppimäärä",
    "GA"->"Portugali, pitkä oppimäärä",
    "GB"->"Portugali, keskipitkä oppimäärä",
    "GC"->"Portugali, lyhyt oppimäärä",
    "HA"->"Unkari, pitkä oppimäärä",
    "HB"->"Unkari, keskipitkä oppimäärä",
    "I"->"Äidinkielen koe, inarinsaame",
    "W"->"Äidinkielen koe, koltansaame",
    "IC"->"Inarinsaame, lyhyt oppimäärä",
    "QC"->"Koltan saame, lyhyt oppimäärä",
    "J"->"Englanninkielinen kypsyyskoe",
    "KC"->"Kreikka, lyhyt oppimäärä",
    "L1"->"Latina, lyhyt oppimäärä",
    "L7"->"Latina, laajempi oppimäärä",
    "M"->"Matematiikan koe, pitkä oppimäärä",
    "N"->"Matematiikan koe, lyhyt oppimäärä",
    "PA"->"Espanja, pitkä oppimäärä",
    "PB"->"Espanja, keskipitkä oppimäärä",
    "PC"->"Espanja, lyhyt oppimäärä",
    "RR"->"Reaali, ev lut uskonnon kysymykset",
    "RO"->"Reaali, ortod.uskonnon kysymykset",
    "RY"->"Reaali, elämänkatsomustiedon kysymykset",
    "SA"->"Saksa, pitkä oppimäärä",
    "SB"->"Saksa, keskipitkä oppimäärä",
    "SC"->"Saksa, lyhyt oppimäärä",
    "S9"->"Saksalaisen koulun saksan kielen koe",
    "TA"->"Italia, pitkä oppimäärä",
    "TB"->"Italia, keskipitkä oppimäärä",
    "TC"->"Italia, lyhyt oppimäärä",
    "VA"->"Venäjä, pitkä oppimäärä",
    "VB"->"Venäjä, keskipitkä oppimäärä",
    "VC"->"Venäjä, lyhyt oppimäärä",
    "Z"->"Äidinkielen koe, pohjoissaame",
    "UE"->"Ev.lut. Uskonto",
    "UO"->"Ortodoksiuskonto",
    "ET"->"Elämänkatsomustieto",
    "FF"->"Filosofia",
    "PS"->"Psykologia",
    "HI"->"Historia",
    "FY"->"Fysiikka",
    "KE"->"Kemia",
    "BI"->"Biologia",
    "GE"->"Maantiede",
    "TE"->"Terveystieto",
    "YH"->"Yhteiskuntaoppi",
    "AINEREAALI"->"Ainereaali",
    "REAALI"->"Reaali",
    "PITKA_KIELI"->"Kieli, pitkä oppimäärä",
    "KESKIPITKA_KIELI"->"Kieli, keskipitkä oppimäärä",
    "LYHYT_KIELI"->"Kieli, lyhyt oppimäärä",
    "AIDINKIELI"->"Äidinkieli"
  )

  val LUKUKAUDET = Array(
    "1" -> "Syksy",
    "2" -> "Kevät"
  )

  val OSAKOKEET = Array(
    "01" -> "Ev.lut. uskonto",
    "02" -> "Ortodoksiuskonto",
    "03" -> "Elämänkatsomustieto",
    "04" -> "Filosofia",
    "05" -> "Psykologia",
    "06" -> "Historia ja yhteiskunta-oppi",
    "07" -> "Fysiikka",
    "08" -> "Kemia",
    "09" -> "Biologia",
    "10" -> "Maantiede"
  )

  val KOEROOLIT = Array(
    "11" -> "Äidinkieli",
    "12" -> "Äidinkieli, saame",
    "13" -> "Kypsyyskoe",
    "14" -> "Äidinkielen tilalla suoritettava suomen tai ruotsin koe",
    "21" -> "Pakollinen toisen kotimaisen kielen koe tai erivapaudella suoritettava muu kieli tai reaali tai matematiikka",
    "22" -> "Pakollinen toisen kotimaisen sijaan suoritettu äidinkieli tässä kielessä",
    "31" -> "Pakollisena kokeena suoritettu vieras kieli",
    "32" -> "Pakollisena kokeen suoritettu vieras kieli",
    "41" -> "Pakollisena kokeena suoritettu reaaliaine",
    "42" -> "Pakollisena kokeena suoritettu matematiikka",
    "60" -> "Ylimääräinen äidinkieli",
    "61" -> "Ylimääräisenä kokeena suoritettu vieras kieli",
    "62" -> "Ylimääräisenä suoritettu toisen kotimaisen kielen koe",
    "71" -> "Ylimääräisenä kokeena suoritettu reaaliaine",
    "81" -> "Ylimääräisenä kokeena suoritettu matematiikka"
  )

  val TOTUUSARVOT = Array(
    "true" -> "true",
    "false" -> "false"
  )

}
