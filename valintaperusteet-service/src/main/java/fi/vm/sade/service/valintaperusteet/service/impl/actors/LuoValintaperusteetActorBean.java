...
public class LuoValintaperusteetActorBean extends UntypedActor {
  ...
  public void onReceive(Object message) {
    if (message instanceof LuoValintaperuste) {
      ...
      if (nimi.contains(", pk")) {
        valintaryhma =
            modelMapper.map(
                valintaryhmaService.insert(valintaryhma, peruste.getpOid()), ValintaryhmaDTO.class);
      } else {
        valintaryhma =
            modelMapper.map(
                valintaryhmaService.insert(valintaryhma, peruste.getlOid()), ValintaryhmaDTO.class);
      }
      transactionManager.commit(tx);
      tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
      ValinnanVaihe valintakoevaihe =
          valinnanVaiheService.findByValintaryhma(valintaryhma.getOid()).get(1);
      ...
      transactionManager.commit(tx);
      tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
      boolean onPoikkeavaRyhma =
          LuoValintaperusteetServiceImpl.poikkeavatValintaryhmat.contains(
              peruste.getHakukohdekoodi().getUri());
      ...
      valinnanVaihe =
          modelMapper.map(
              valinnanVaiheService.lisaaValinnanVaiheValintaryhmalle(
                  valintaryhma.getOid(), valinnanVaihe, valintakoevaihe.getOid()),
      ...
      if (onPoikkeavaRyhma) {
        ...
        valintakoekaava =
            laskentakaavaService.insert(
                PkJaYoPohjaiset.luoValintakoekaava(valintakoeKaavanTunniste, lahde, true),
                null,
                valintaryhma.getOid());
      } else {
        ...
        valintakoekaava =
            laskentakaavaService.insert(
                PkJaYoPohjaiset.luoValintakoekaava(valintakoeKaavanTunniste, lahde),
                null,
                valintaryhma.getOid());
      }
      transactionManager.commit(tx);
      ...
      if (onPoikkeavaRyhma) {
        ensisijainenJarjestyskriteeri =
            PkJaYoPohjaiset.luoPoikkeavanValintaryhmanLaskentakaava(
                valintakoekaava, peruste.getKielikoelaskentakaava());
      } else {
        ensisijainenJarjestyskriteeri =
            PkJaYoPohjaiset.luoYhdistettyPeruskaavaJaValintakoekaava(peruskaava, valintakoekaava);
      }
      transactionManager.commit(tx);
      insertKoe(
          valintaryhma,
      ...
      insertEiKoetta(
          ...
          tasasijakriteerit,
          peruste.getHakukohdekoodi(),
          peruste.getLisapisteLaskentakaava());
  ...
  private void insertKoe(
      ValintaryhmaDTO valintaryhma,
      String valintakoetunniste,
      Laskentakaava peruskaavaJaValintakoekaava,
      Laskentakaava valintakoekaava,
      Laskentakaava[] tasasijakriteerit,
    ...
    koevalintaryhma =
        modelMapper.map(
            valintaryhmaService.insert(koevalintaryhma, valintaryhma.getOid()),
            ValintaryhmaDTO.class);
    transactionManager.commit(tx);
    tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
    hakukohdekoodiService.lisaaHakukohdekoodiValintaryhmalle(
        koevalintaryhma.getOid(), hakukohdekoodi);
    transactionManager.commit(tx);
    tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
    peruskaavaJaValintakoekaava =
        laskentakaavaService.insert(peruskaavaJaValintakoekaava, null, koevalintaryhma.getOid());
    transactionManager.commit(tx);
    tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
    // Aktivoidaan pääsykoe
    List<ValinnanVaihe> valinnanVaiheet =
        valinnanVaiheService.findByValintaryhma(koevalintaryhma.getOid());
    ValinnanVaihe valintakoevaihe = valinnanVaiheet.get(1);
    assert (valintakoevaihe
    ...
    List<Valintakoe> valintakokeet =
        valintakoeService.findValintakoeByValinnanVaihe(valintakoevaihe.getOid());
    ...
    valintakoekoodiService.lisaaValintakoekoodiValintaryhmalle(
        koevalintaryhma.getOid(), valintakoekoodi);
    transactionManager.commit(tx);
    tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
    ...
    Valintatapajono jono =
        valintatapajonoService.findJonoByValinnanvaihe(tavallinenVaihe.getOid()).get(0);
    transactionManager.commit(tx);
    tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
    ...
    jarjestyskriteeriService.lisaaJarjestyskriteeriValintatapajonolle(
        jono.getOid(), kriteeri, null, peruskaavaJaValintakoekaava.getId());
    transactionManager.commit(tx);
    tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
    for (int i = 0; i < tasasijakriteerit.length; ++i) {
      if (i == 1) {
        ...
        jarjestyskriteeriService.lisaaJarjestyskriteeriValintatapajonolle(
            jono.getOid(), jk, null, valintakoekaava.getId());
      }
      ...
      jarjestyskriteeriService.lisaaJarjestyskriteeriValintatapajonolle(
          jono.getOid(), jk, null, kaava.getId());
    }
    transactionManager.commit(tx);
  }

  private void insertEiKoetta(
      ValintaryhmaDTO valintaryhma,
    ...
    koe =
        modelMapper.map(
            valintaryhmaService.insert(koe, valintaryhma.getOid()), ValintaryhmaDTO.class);
    transactionManager.commit(tx);
    tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
    ...
    boolean onPoikkeavaRyhma =
        LuoValintaperusteetServiceImpl.poikkeavatValintaryhmatLisapisteilla.contains(
            hakukohdekoodi.getUri());
    Laskentakaava lisattava = null;
    if (onPoikkeavaRyhma) {
      lisattava =
          laskentakaavaService.insert(
              PkJaYoPohjaiset.luoYhdistettyPeruskaavaJaLisapistekaava(peruskaava, lisapistekaava),
              null,
              koe.getOid());
      transactionManager.commit(tx);
      tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
    ...
    jarjestyskriteeriService.lisaaJarjestyskriteeriValintatapajonolle(
        jono.getOid(), kriteeri, null, lisattava.getId());
    transactionManager.commit(tx);
    tx = transactionManager.getTransaction(new DefaultTransactionDefinition());
    for (int i = 0; i < tasasijakriteerit.length; ++i) {
      Laskentakaava kaava = tasasijakriteerit[i];
