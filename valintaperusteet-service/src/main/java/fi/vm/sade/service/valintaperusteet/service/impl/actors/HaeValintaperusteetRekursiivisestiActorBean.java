...
public class HaeValintaperusteetRekursiivisestiActorBean extends UntypedActor {
    ...
    final static private Logger LOGGER = LoggerFactory.getLogger(HaeValintaperusteetRekursiivisestiActorBean.class.getName());
            ...
            if(original.getFunktioargumentit() == null || original.getFunktioargumentit().size() == 0) {
                ...
                for (Funktioargumentti arg : original.getFunktioargumentit()) {
                    ...
                    if (arg.getFunktiokutsuChild() != null) {
                        child.tell(new UusiValintaperusteRekursio(arg.getFunktiokutsuChild().getId(),
                                viesti.getValintaperusteet(), viesti.getHakukohteenValintaperusteet()),self());
                    } else if (arg.getLaskentakaavaChild() != null) {
                        child.tell(new UusiValintaperusteRekursio(arg.getLaskentakaavaChild().getFunktiokutsu().getId(),
                                viesti.getValintaperusteet(), viesti.getHakukohteenValintaperusteet()),self());
                    }
                }
            }
