...
public class HakukohdeImportServiceImpl implements HakukohdeImportService {
  ...
  private Valintaryhma selvitaValintaryhma(HakukohdeImportDTO importData) {
    ...
    String hakukohdekoodi = sanitizeKoodiUri(importData
        .getHakukohdekoodi()
        .getKoodiUri());
    Set<String> valintakoekoodit = importData
        .getValintakoe()
        .stream()
    ...
    if (valintaryhmat.size() != 1) {
      LOG.info("Haku {}, hakukohdekoodi {} ja valintakoekoodit {} eivät antaneet yksikäsitteistä valintaryhmää. Mahdolliset ryhmät {}.",
          hakuOid,
          hakukohdekoodi,
    ...
    if (valintaryhmat.size() != 1) {
      LOG.info("Haku {} ei antanut yksikäsitteistä valintaryhmää. Mahdolliset ryhmät {}.",
          hakuOid,
          valintaryhmat.stream().map(Valintaryhma::getOid).collect(Collectors.joining(", ")));
    ...
    if (valintaryhma == null) {
      LOG.info(
          "Hakukohteen {} tulisi olla juurivalintaryhmän alla.", hakukohdeOid);
    } else {
      LOG.info(
          "Hakukohteen {} tulisi olla valintaryhmän {} alla.",
          hakukohdeOid,
          valintaryhma.getOid());
    }
