...
public class LaskentakaavaResourceImpl implements LaskentakaavaResource {
    ...
    public Response siirra(LaskentakaavaSiirraDTO dto, @Context HttpServletRequest request) {
        ...
        return siirretty.map(kaava -> {
            AuditLog.log(ValintaperusteetAudit.AUDIT, AuditLog.getUser(request), ValintaperusteetOperation.LASKENTAKAAVA_SIIRTO, ValintaResource.LASKENTAKAAVA, Long.toString(kaava.getId()), Changes.addedDto(dto));
