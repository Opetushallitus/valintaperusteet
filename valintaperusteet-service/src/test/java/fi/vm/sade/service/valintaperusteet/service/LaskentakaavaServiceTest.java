...
public class LaskentakaavaServiceTest {
	...
	private static class FunktioArgumenttiComparator implements
			Comparator<Funktioargumentti> {
	...
	public void initialize() {
		try {
			...
			Connection conn = DriverManager.getConnection(
					"jdbc:hsqldb:mem:valintaperusteet", "sa", "");
	...
	public void testHaeKaava() {
		...
		List<Funktioargumentti> maksimi204Largs = argsSorted(maksimi204L
		...
		List<Funktioargumentti> summa203Largs = argsSorted(summa203L
				.getFunktioargumentit());
		...
		List<Funktioargumentti> summa201Largs = argsSorted(summa201L
				.getFunktioargumentit());
		List<Funktioargumentti> tulo202LLargs = argsSorted(tulo202L
				.getFunktioargumentit());

		Funktiokutsu luku1L = summa201Largs.get(0).getFunktiokutsuChild();
		Funktiokutsu luku2L = summa201Largs.get(1).getFunktiokutsuChild();
		Funktiokutsu haeLukuarvo405L = summa201Largs.get(2)
		...
		ValintaperusteViite haeLukuarvo405LValintaperuste = haeLukuarvo405L
				.getValintaperusteviitteet().iterator().next();
	...
	private FunktiokutsuDTO createLukuarvo(Double luku) {
		...
		final Funktiokuvaaja.Funktiokuvaus funktiokuvaus = Funktiokuvaaja
				.annaFunktiokuvaus(nimi)._2();

		FunktiokutsuDTO funktiokutsu = new FunktiokutsuDTO();
		funktiokutsu
				.setFunktionimi(fi.vm.sade.service.valintaperusteet.dto.model.Funktionimi.LUKUARVO);
	...
	private FunktiokutsuDTO createSumma(FunktiokutsuDTO... args) {
		FunktiokutsuDTO funktiokutsu = new FunktiokutsuDTO();
		funktiokutsu
				.setFunktionimi(fi.vm.sade.service.valintaperusteet.dto.model.Funktionimi.SUMMA);
		funktiokutsu.setTallennaTulos(false);

		for (int i = 0; i < args.length; ++i) {
			FunktioargumentinLapsiDTO f = modelMapper.map(args[i],
					FunktioargumentinLapsiDTO.class);
	...
	public void testInsertNew() {
		...
		laskentakaava.setFunktiokutsu(createSumma(createLukuarvo(5.0),
				createLukuarvo(10.0), createLukuarvo(100.0)));
		...
		assertEquals(Funktionimi.SUMMA, haettu.getFunktiokutsu()
				.getFunktionimi());
		assertEquals(3, haettu.getFunktiokutsu().getFunktioargumentit().size());

		for (Funktioargumentti fa : haettu.getFunktiokutsu()
				.getFunktioargumentit()) {
			assertEquals(Funktionimi.LUKUARVO, fa.getFunktiokutsuChild()
					.getFunktionimi());
		}
	}
	...
	private double luku(Funktiokutsu lukufunktio) {
		...
		Syoteparametri syoteparametri = lukufunktio.getSyoteparametrit()
				.iterator().next();
	...
	public void testUpdate() {
		...
		Laskentakaava paivitetty = null;
		{
			...
			List<Funktioargumentti> summa500Largs = argsSorted(summa500L
					.getFunktioargumentit());
			...
			Funktiokutsu summa503L = summa500Largs.get(2)
					.getFunktiokutsuChild();
			Funktiokutsu summa506L = summa500Largs.get(3)
					.getFunktiokutsuChild();
			...
			List<Funktioargumentti> summa503Largs = argsSorted(summa503L
					.getFunktioargumentit());
			Funktiokutsu haeMerkkijonoJaKonvertoiLukuarvoksi504L = summa503Largs
					.get(0).getFunktiokutsuChild();
			...
			assertEquals(5, haeMerkkijonoJaKonvertoiLukuarvoksi504L
					.getArvokonvertteriparametrit().size());

			List<Funktioargumentti> summa506Largs = argsSorted(summa506L
					.getFunktioargumentit());
			...
			Syoteparametri lukuparam = luku501L.getSyoteparametrit().iterator()
					.next();
			...
			Arvokonvertteriparametri param = haeMerkkijonoJaKonvertoiLukuarvoksi504L
					.getArvokonvertteriparametrit().iterator().next();
			haeMerkkijonoJaKonvertoiLukuarvoksi504L
					.getArvokonvertteriparametrit().remove(param);
			...
			LaskentakaavaCreateDTO dto = modelMapper.map(laskentakaava,
					LaskentakaavaCreateDTO.class);
			paivitetty = laskentakaavaService
					.update(laskentakaava.getId(), dto);
		...
		List<Funktioargumentti> summa500Lpargs = argsSorted(summa500Lp
				.getFunktioargumentit());
		...
		List<Funktioargumentti> summa503Lpargs = argsSorted(summa503Lp
				.getFunktioargumentit());
		Funktiokutsu haeMerkkijonoJaKonvertoiLukuarvoksi504Lp = summa503Lpargs
				.get(0).getFunktiokutsuChild();
	...
	private FunktiokutsuDTO nimettyFunktiokutsu(String nimi,
			FunktiokutsuDTO child) {
		FunktiokutsuDTO nimetty = new FunktiokutsuDTO();
		nimetty.setTallennaTulos(false);

		FunktioargumenttiDTO arg = new FunktioargumenttiDTO();
		FunktioargumentinLapsiDTO lapsiDTO = modelMapper.map(child,
				FunktioargumentinLapsiDTO.class);
		...
		if (Funktiotyyppi.LUKUARVOFUNKTIO.equals(child.getFunktionimi()
				.getTyyppi())) {
	...
	public void lisaaKaavaJokaViittaaToiseenKaavaan() {
		...
		Laskentakaava tallennettu = null;
		{
			...
			LaskentakaavaListDTO tallennettuKaava = modelMapper.map(
					laskentakaavaService.read(tallennettuKaavaId),
					LaskentakaavaListDTO.class);

			FunktiokutsuDTO summa = createSumma(createLukuarvo(1.0),
					createLukuarvo(2.0));
			FunktioargumenttiDTO kaavaArg = new FunktioargumenttiDTO();
			FunktioargumentinLapsiDTO lapsiDTO = modelMapper.map(
					tallennettuKaava, FunktioargumentinLapsiDTO.class);
			...
			FunktiokutsuDTO nimettyFunktiokutsu = nimettyFunktiokutsu(nimi,
					summa);
			...
			tallennettu = laskentakaavaService
					.insert(laskentakaava, null, null);
		}
		...
		Funktioargumentti nimettyArg = nimetty.getFunktioargumentit()
				.iterator().next();
		...
		List<Funktioargumentti> summaArgs = argsSorted(summa
				.getFunktioargumentit());
		...
		assertEquals(Funktionimi.LUKUARVO, summaArgs.get(0)
				.getFunktiokutsuChild().getFunktionimi());
		assertEquals(Funktionimi.LUKUARVO, summaArgs.get(1)
				.getFunktiokutsuChild().getFunktionimi());
		assertEquals(laskentakaavaService.read(tallennettuKaavaId), summaArgs
				.get(2).getLaskentakaavaChild());
	}
	...
	public void testHaeLaskettavaKaava()
		...
		{
			Funktiokutsu funktiokutsu = laskentakaavaService
					.haeMallinnettuFunktiokutsu(tulo);
			assertEquals(tulo, new Long(funktiokutsu.getId()));
			assertEquals(Funktionimi.TULO, funktiokutsu.getFunktionimi());
			assertEquals(2, funktiokutsu.getFunktioargumentit().size());

			List<Funktioargumentti> args = argsSorted(funktiokutsu
					.getFunktioargumentit());
	...
	public void haeMallinnettuLaskentakaava() {
		...
		Laskentakaava laskentakaava = laskentakaavaService.haeLaskettavaKaava(
				laskentakaavaId, Laskentamoodi.VALINTALASKENTA);
		Funktiokutsu nimetty513L = laskentakaava.getFunktiokutsu();

		assertEquals(Funktionimi.NIMETTYLUKUARVO, nimetty513L.getFunktionimi());
		assertEquals(1, nimetty513L.getFunktioargumentit().size());

		Funktiokutsu tulo512L = argsSorted(nimetty513L.getFunktioargumentit())
				.get(0).getFunktiokutsuChild();
		assertEquals(Funktionimi.TULO, tulo512L.getFunktionimi());
		assertEquals(2, tulo512L.getFunktioargumentit().size());
		...
		List<Funktioargumentti> tulo512Largs = argsSorted(tulo512L
				.getFunktioargumentit());
		Laskentakaava laskentakaava509L = tulo512Largs.get(0)
				.getLaskentakaavaChild();
		Funktiokutsu luku511L = tulo512Largs.get(1).getFunktiokutsuChild();

		assertEquals(Funktiotyyppi.LUKUARVOFUNKTIO,
				laskentakaava509L.getTyyppi());
		assertEquals(Funktionimi.LUKUARVO, luku511L.getFunktionimi());
	}

	@Test
	public void testFindAvaimetForHakukohdes() {
		List<ValintaperusteDTO> valintaperusteet = laskentakaavaService
				.findAvaimetForHakukohde("oid17");
		...
		assertEquals(new BigDecimal("0.0"), new BigDecimal(valintaperusteet
				.get(1).getMin()));
		assertEquals(new BigDecimal("30.0"), new BigDecimal(valintaperusteet
				.get(1).getMax()));
		assertNull(valintaperusteet.get(1).getArvot());
		assertEquals(Valintaperustelahde.SYOTETTAVA_ARVO,
				valintaperusteet.get(1).getLahde());

	}

	@Test
	public void testFindHakukohdeValintaperusteAvaimet() {
		HakukohteenValintaperusteAvaimetDTO valintaperusteet = laskentakaavaService
				.findHakukohteenAvaimet("oid23");
	...
	public void testItseensaViittaavaKaava() {
		...
		{
			Laskentakaava ylakaava = laskentakaavaService.haeLaskettavaKaava(
					ylakaavaId, Laskentamoodi.VALINTALASKENTA);
			assertEquals(Funktionimi.SUMMA, ylakaava.getFunktiokutsu()
					.getFunktionimi());

			Funktiokutsu ylaFunktiokutsu = ylakaava.getFunktiokutsu();
			assertEquals(2, ylaFunktiokutsu.getFunktioargumentit().size());

			List<Funktioargumentti> ylafunktioArgs = argsSorted(ylaFunktiokutsu
					.getFunktioargumentit());
			assertNotNull(ylafunktioArgs.get(1).getLaskentakaavaChild());
			assertEquals(alakaavaId, new Long(ylafunktioArgs.get(1)
					.getLaskentakaavaChild().getId()));

		}
		{
			Laskentakaava alakaava = laskentakaavaService.haeLaskettavaKaava(
					alakaavaId, Laskentamoodi.VALINTALASKENTA);
			assertEquals(Funktionimi.SUMMA, alakaava.getFunktiokutsu()
					.getFunktionimi());

			Funktiokutsu alaFunktiokutsu = alakaava.getFunktiokutsu();
			assertEquals(2, alaFunktiokutsu.getFunktioargumentit().size());

			List<Funktioargumentti> alafunktioArgs = argsSorted(alaFunktiokutsu
					.getFunktioargumentit());
		...
		try {
			laskentakaavaService.update(alakaavaId,
					modelMapper.map(alakaava, LaskentakaavaCreateDTO.class));
	...
	public void testHaeLaskettavaKaavaVaarallaMoodilla() {
		...
		laskentakaavaService.haeLaskettavaKaava(laskentakaavaId,
	...
	public void testSiirra() {
		...
		LaskentakaavaSiirraDTO siirrettava = modelMapper.map(tallennettu,
				LaskentakaavaSiirraDTO.class);
		...
		assertEquals(Funktionimi.SUMMA, siirretty.getFunktiokutsu()
				.getFunktionimi());
		assertEquals(3, siirretty.getFunktiokutsu().getFunktioargumentit()
				.size());

		for (Funktioargumentti fa : siirretty.getFunktiokutsu()
				.getFunktioargumentit()) {
			assertEquals(Funktionimi.LUKUARVO, fa.getFunktiokutsuChild()
					.getFunktionimi());
		}
	...
	public void testSiirraValintaRyhmaaEiLoydy() {
		...
		Optional<Laskentakaava> siirretty = laskentakaavaService
				.siirra(siirrettava);
