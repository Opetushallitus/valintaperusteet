...
public class FunktiokutsuDAOTest {
	...
	public void testGetLukuarvo() {
		...
		assertEquals(
				fi.vm.sade.service.valintaperusteet.dto.model.Funktionimi.LUKUARVO,
	...
	public void testGetSumma() {
		...
		List<Funktioargumentti> args = new ArrayList<Funktioargumentti>(
				funktiokutsu.getFunktioargumentit());
		...
		assertEquals(Funktionimi.LUKUARVO, args.get(0).getFunktiokutsuChild()
				.getFunktionimi());
		assertEquals(Funktionimi.LUKUARVO, args.get(1).getFunktiokutsuChild()
				.getFunktionimi());
		assertEquals(Funktionimi.HAELUKUARVO, args.get(2)
	...
	public void testGetLukuarvoluvuksi() {
		final Long id = 301L;
		Funktiokutsu funktiokutsu = funktiokutsuDAO.getFunktiokutsu(id);
		assertEquals(Funktionimi.KONVERTOILUKUARVO,
				funktiokutsu.getFunktionimi());
		assertEquals(1, funktiokutsu.getFunktioargumentit().size());

		Funktiokutsu arg = funktiokutsu.getFunktioargumentit().iterator()
				.next().getFunktiokutsuChild();
	...
	public void testDeleteOrphans() {
		...
		assertEquals(101L, orphans.get(0).getId());
		assertEquals(301L, orphans.get(1).getId());
		assertEquals(302L, orphans.get(2).getId());
	...
	public void testFindFunktiokutsuByHakukohdeOids() {
		List<Funktiokutsu> kaavat = funktiokutsuDAO
				.findFunktiokutsuByHakukohdeOids("oid17");
	...
	public void testGetHylkaysperuste() {
		...
		for (Arvokonvertteriparametri ak : funktiokutsu
