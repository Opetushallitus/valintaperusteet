...
public class ValintakoeServiceTest {
	...
	public void testLisaaValintakoeValinnanVaiheelle() {
		...
		{
			ValinnanVaihe vrValinnanVaihe = valinnanVaiheService
					.readByOid(valinnanVaiheOid);
			List<Valintakoe> vrValinnanVaihekokeet = valintakoeService
					.findValintakoeByValinnanVaihe(valinnanVaiheOid);
			assertEquals(1, vrValinnanVaihekokeet.size());

			ValinnanVaihe hkValinnanVaihe = valinnanVaiheService
					.readByOid(kopioValinnanVaiheOid);
			assertEquals(vrValinnanVaihe.getOid(), hkValinnanVaihe.getMaster()
					.getOid());
			List<Valintakoe> hkValinnanVaiheKokeet = valintakoeService
					.findValintakoeByValinnanVaihe(hkValinnanVaihe.getOid());
			assertEquals(1, hkValinnanVaiheKokeet.size());
			assertEquals(vrValinnanVaihekokeet.get(0).getOid(),
					hkValinnanVaiheKokeet.get(0).getMaster().getOid());
		}
		...
		valintakoeService.lisaaValintakoeValinnanVaiheelle(valinnanVaiheOid,
				valintakoe);

		{
			...
			assertEquals(vrValinnanVaihekokeet.get(0).getOid(),
					hkValinnanVaiheKokeet.get(0).getMaster().getOid());
			assertEquals(vrValinnanVaihekokeet.get(1).getOid(),
					hkValinnanVaiheKokeet.get(1).getMaster().getOid());

			assertEquals(vrValinnanVaihekokeet.get(1).getAktiivinen(),
					valintakoe.getAktiivinen());
			assertEquals(vrValinnanVaihekokeet.get(1).getKuvaus(),
					valintakoe.getKuvaus());
			assertEquals(vrValinnanVaihekokeet.get(1).getNimi(),
					valintakoe.getNimi());
			assertEquals(vrValinnanVaihekokeet.get(1).getTunniste(),
					valintakoe.getTunniste());
			assertEquals(new Long(vrValinnanVaihekokeet.get(1)
					.getLaskentakaava().getId()),
					valintakoe.getLaskentakaavaId());

			assertEquals(hkValinnanVaiheKokeet.get(1).getAktiivinen(),
					valintakoe.getAktiivinen());
			assertEquals(hkValinnanVaiheKokeet.get(1).getKuvaus(),
					valintakoe.getKuvaus());
			assertEquals(hkValinnanVaiheKokeet.get(1).getNimi(),
					valintakoe.getNimi());
			assertEquals(hkValinnanVaiheKokeet.get(1).getTunniste(),
					valintakoe.getTunniste());
			assertEquals(new Long(hkValinnanVaiheKokeet.get(1)
					.getLaskentakaava().getId()),
					valintakoe.getLaskentakaavaId());
		}
	}
	...
	public void testUpdate() {
		...
		assertFalse(new Long(managed.getLaskentakaava().getId()) == update
				.getLaskentakaavaId());
		...
		assertEquals(update.getLaskentakaavaId(), new Long(managed
				.getLaskentakaava().getId()));
		...
		assertEquals(update.getLaskentakaavaId(), new Long(kopio
				.getLaskentakaava().getId()));
	}
