...
@ContextConfiguration(
    locations =
        "classpath:fi/vm/sade/service/valintaperusteet/service/integration-test-context.xml")
...
public class LaskentakaavaServiceIntegrationTestToBeRunManually {
  ...
  public void testFindAvaimetForHakukohde() {
    HakukohdeWithValintaperusteet hakukohdeWithValintaperusteet =
        new HakukohdeWithValintaperusteet.Builder("1.2.246.562.20.39188224891")
            .addValintaperuste(
                "5454fd76-0e02-e868-4e45-f96061aa0cc0",
                "Entrance examination: 0-70 points, min. 30 points.")
            .addValintaperuste(
                "6b3c52e0-f9e7-7a75-8b83-05611208fa3f",
                "Pre-assignment: 1 = delivered, 2 = not delivered.")
            .addValintaperuste(
                "ced128ef-f288-fa6a-af47-3a0fa1c3a887",
                "Pre-assignment: 0-30 points, min. 10 points")
            .build();

    for (int i = 0; i < 10; i++) {
      assertThat(
          valintaperusteetFor(hakukohdeWithValintaperusteet.hakukohdeOid),
          are(hakukohdeWithValintaperusteet));
    }
  }
    ...
    return new TypeSafeMatcher<List<ValintaperusteDTO>>() {
      ...
      protected boolean matchesSafely(List<ValintaperusteDTO> item) {
        ...
        for (ValintaperusteDTO valintaperusteKannasta : item) {
          ...
          if (!odotettuTunnisteJaKuvaus.getLeft().equals(valintaperusteKannasta.getTunniste())) {
            failure =
                String.format(
                    "tunniste at position %d: expected %s but was %s",
          ...
          if (!odotettuTunnisteJaKuvaus.getRight().equals(valintaperusteKannasta.getKuvaus())) {
            failure =
                String.format(
                    "kuvaus at position %d: expected %s but was %s",
      ...
      public void describeTo(Description description) {
        description.appendText(
            String.format(
                "list with valintaperusteet %s : %s", expected.tunnnisteetJaKuvaukset, failure));
      }
    };
  }
  ...
  public static class HakukohdeWithValintaperusteet {
    ...
    private HakukohdeWithValintaperusteet(
        String hakukohdeOid, List<Pair<String, String>> tunnnisteetJaKuvaukset) {
