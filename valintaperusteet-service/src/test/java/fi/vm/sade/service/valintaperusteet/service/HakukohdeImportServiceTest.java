...
public class HakukohdeImportServiceTest {
	...
	private MonikielinenTekstiDTO luoMonikielinenTeksti(String teksti,
			HakukohdeImportServiceImpl.Kieli kieli) {
	...
	private HakukohdeImportDTO luoHakukohdeImportDTO(String hakukohdeOid,
			String hakuOid, String koodiUri) {
		HakukohdeImportDTO imp = new HakukohdeImportDTO();
		imp.setHakukohdeOid(hakukohdeOid);
		imp.setHakuOid(hakuOid);
		imp.getHakukohdeNimi().add(
				luoMonikielinenTeksti(hakukohdeOid + "-nimi",
						HakukohdeImportServiceImpl.Kieli.FI));
		imp.getTarjoajaNimi().add(
				luoMonikielinenTeksti(hakukohdeOid + "-tarjoaja",
						HakukohdeImportServiceImpl.Kieli.FI));
		imp.getHakuKausi().add(
				luoMonikielinenTeksti("Syksy",
						HakukohdeImportServiceImpl.Kieli.FI));
	...
	public void testImportNewHakukohde() {
		...
		HakukohdeImportDTO importData = luoHakukohdeImportDTO(hakukohdeOid,
				hakuOid, koodiUri);
		...
		List<HakukohdeViite> hakukohteet = hakukohdeViiteDAO
				.readByHakukohdekoodiUri(koodiUri);
	...
	public void testImportHakukohdeWithExistingValintaryhma() {
		...
		List<Valintaryhma> valintaryhmas = valintaryhmaDAO
				.readByHakukohdekoodiUri(koodiUri);
		assertEquals(1, valintaryhmas.size());
		assertEquals(valintaryhmaOid, valintaryhmas.get(0).getOid());

		List<HakukohdeViite> hakukohdeViites = hakukohdeViiteDAO
				.readByHakukohdekoodiUri(koodiUri);
		assertEquals(0, hakukohdeViites.size());
		assertNull(hakukohdeViiteDAO.readByOid(hakukohdeOid));

		assertEquals(0, hakukohdeViiteDAO
				.findByValintaryhmaOid(valintaryhmaOid).size());
		...
		List<HakukohdeViite> hakukohteetByValintaryhma = hakukohdeViiteDAO
				.findByValintaryhmaOid(valintaryhmaOid);
	...
	public void testImportHakukohdeUnderNewValintaryhma() {
		...
		{
			List<Valintaryhma> valintaryhmas = valintaryhmaDAO
					.readByHakukohdekoodiUri(hakukohdekoodiUri);
			List<HakukohdeViite> hakukohdeViites = hakukohdeViiteDAO
					.readByHakukohdekoodiUri(hakukohdekoodiUri);
			assertEquals(1, valintaryhmas.size());
			...
			List<HakukohdeViite> hakukohteet = hakukohdeService
					.findByValintaryhmaOid(valintaryhmaOidAluksi);
			assertEquals(1, hakukohteet.size());
			...
			assertEquals(
					0,
					hakukohdeService.findByValintaryhmaOid(
							valintaryhmaOidLopuksi).size());

			List<ValinnanVaihe> vaiheet = valinnanVaiheService
					.findByHakukohde(hakukohdeOid);
			assertEquals(4, vaiheet.size());
			assertTrue(vaiheet.get(0).getId() == 95L
					&& vaiheet.get(0).getMasterValinnanVaihe().getId() == 94L);
			assertTrue(vaiheet.get(1).getId() == 96L
					&& vaiheet.get(1).getMasterValinnanVaihe() == null);
			assertTrue(vaiheet.get(2).getId() == 97L
					&& vaiheet.get(2).getMasterValinnanVaihe().getId() == 93L);
			assertTrue(vaiheet.get(3).getId() == 98L
					&& vaiheet.get(3).getMasterValinnanVaihe() == null);
		}

		HakukohdeImportDTO importData = luoHakukohdeImportDTO(hakukohdeOid,
				hakuOid, hakukohdekoodiUri);
		hakukohdeImportService.tuoHakukohde(importData);

		{
			...
			assertEquals(
					0,
					hakukohdeService.findByValintaryhmaOid(
							valintaryhmaOidAluksi).size());
			List<HakukohdeViite> hakukohteet = hakukohdeService
					.findByValintaryhmaOid(valintaryhmaOidLopuksi);
			assertEquals(1, hakukohteet.size());
			assertEquals(hakukohdeOid, hakukohteet.get(0).getOid());
			...
			assertTrue(vaiheet.get(0).getMasterValinnanVaihe().getId() == 99L);
			assertTrue(vaiheet.get(1).getId() == 96L
					&& vaiheet.get(1).getMasterValinnanVaihe() == null);
			assertTrue(vaiheet.get(2).getId() == 98L
					&& vaiheet.get(2).getMasterValinnanVaihe() == null);
		}
	}
	...
	public void testImportHakukohdeOutsideValintaryhma() {
...

		{
			...
			assertTrue(vaiheet.get(0).getId() == 96L
					&& vaiheet.get(0).getMasterValinnanVaihe() == null);
			assertTrue(vaiheet.get(1).getId() == 98L
					&& vaiheet.get(1).getMasterValinnanVaihe() == null);
		}
	}
	...
	public void testKaksiValintaryhmaaSamoillaKoodeilla() {
		...
		{
			assertNull(hakukohdeViiteDAO.readByOid(hakukohdeOid));
			Hakukohdekoodi koodi = hakukohdekoodiDAO
					.readByUri(hakukohdekoodiUri);
			assertNotNull(koodi);
			List<Valintaryhma> valintaryhmas = valintaryhmaDAO
					.readByHakukohdekoodiUri(hakukohdekoodiUri);

			assertEquals(2, valintaryhmas.size());
			List<Valintaryhma> valintaryhmat = new ArrayList<Valintaryhma>(
					valintaryhmas);
			...
			Valintaryhma valintaryhma1 = valintaryhmaService
					.readByOid(valintaryhmaOid1);
			Valintaryhma valintaryhma2 = valintaryhmaService
					.readByOid(valintaryhmaOid2);
			...
			assertTrue(valintaryhma1.getValintakoekoodit().size() == 1
					&& OLETUS_VALINTAKOEURI.equals(valintaryhma1
							.getValintakoekoodit().iterator().next().getUri()));
			assertTrue(valintaryhma2.getValintakoekoodit().size() == 1
					&& OLETUS_VALINTAKOEURI.equals(valintaryhma2
							.getValintakoekoodit().iterator().next().getUri()));
		}
	...
	public void testPaivitaAloituspaikkojenLkm() {
		...
		final int aloituspaikat = 100;
		{
			...
			List<ValinnanVaihe> valinnanVaiheet = valinnanVaiheService
	...
	public void testMelkeinSopivaValintaryhma() {
...

		{
			...
			List<Valintakoekoodi> valintakoekoodit = valintaryhma1
					.getValintakoekoodit();
	...
	public void testLisaaValintaPerusteet() {
		...
		hakukohdeImportService.tuoHakukohde(importData);
		{
			...
			assertEquals("12.0", hakukohde.getHakukohteenValintaperusteet()
					.get("paasykoe_min").getArvo());
			assertEquals("tämä on kielikokeen tunniste", hakukohde
					.getHakukohteenValintaperusteet().get("kielikoe_tunniste")
					.getArvo());
			assertEquals("15.0", hakukohde.getHakukohteenValintaperusteet()
					.get("MU_painokerroin").getArvo());
			assertEquals("35.0", hakukohde.getHakukohteenValintaperusteet()
					.get("A1_EN_painokerroin").getArvo());
			assertEquals("43.0", hakukohde.getHakukohteenValintaperusteet()
					.get("B3_FR_painokerroin").getArvo());
		}
...

		{
			...
			assertEquals(
					"3.0",
					hakukohde.getHakukohteenValintaperusteet()
							.get("A1_EN_painokerroin").getArvo());
			assertEquals(
					"1.0",
					hakukohde.getHakukohteenValintaperusteet()
